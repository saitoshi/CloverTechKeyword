...
RakutenMA.prototype.add_efeats = function (csent) {
  ...
  var _t = function (i) {
    return i >= 0 && i < csent.length ? csent[i] : _empty_token;
  };
  ...
  var add_ctype_feats = function (arr, label, ctype) {
    ...
    if (typeof ctype == "string") {
      ...
      for (i = 0; i < ctype.length; i++) arr.push(_f([label, ctype[i]]));
    }
  };
  ...
  for (i = 0; i < csent.length; i++) {
    ...
    for (j = 0; j < this.featset.length; j++) {
      ...
      switch (feat) {
        ...
        case "c0":
          add_ctype_feats(csent[i].f, "c0", _t(i).t);
          break;
        case "c1":
          add_ctype_feats(csent[i].f, "c1", _t(i + 1).t);
          break;
        case "c9":
          add_ctype_feats(csent[i].f, "c9", _t(i - 1).t);
          break;
        case "c2":
          add_ctype_feats(csent[i].f, "c2", _t(i + 2).t);
          break;
        case "c8":
          add_ctype_feats(csent[i].f, "c8", _t(i - 2).t);
          break;
        case "c3":
          add_ctype_feats(csent[i].f, "c3", _t(i + 3).t);
          break;
        case "c7":
          add_ctype_feats(csent[i].f, "c7", _t(i - 3).t);
          break;
        // character unigram
        case "w0":
          csent[i].f.push(_f(["w0", _t(i).c]));
          break;
        case "w1":
          csent[i].f.push(_f(["w1", _t(i + 1).c]));
          break;
        case "w9":
          csent[i].f.push(_f(["w9", _t(i - 1).c]));
          break;
        case "w2":
          csent[i].f.push(_f(["w2", _t(i + 2).c]));
          break;
        case "w8":
          csent[i].f.push(_f(["w8", _t(i - 2).c]));
          break;
        case "w3":
          csent[i].f.push(_f(["w3", _t(i + 3).c]));
          break;
        case "w7":
          csent[i].f.push(_f(["w7", _t(i - 3).c]));
          break;
        // character bigram
        case "b1":
          csent[i].f.push(_f(["b1", _t(i).c, _t(i + 1).c]));
          break;
        case "b9":
          csent[i].f.push(_f(["b9", _t(i - 1).c, _t(i).c]));
          break;
        case "b2":
          csent[i].f.push(_f(["b2", _t(i + 1).c, _t(i + 2).c]));
          break;
        case "b8":
          csent[i].f.push(_f(["b8", _t(i - 2).c, _t(i - 1).c]));
          break;
        case "b3":
          csent[i].f.push(_f(["b3", _t(i + 2).c, _t(i + 3).c]));
          break;
        case "b7":
          csent[i].f.push(_f(["b7", _t(i - 3).c, _t(i - 2).c]));
          break;
        // character type bigram
        case "d1":
          csent[i].f.push(_f(["d1", _t(i).t, _t(i + 1).t]));
          break;
        case "d9":
          csent[i].f.push(_f(["d9", _t(i - 1).t, _t(i).t]));
          break;
        case "d2":
          csent[i].f.push(_f(["d2", _t(i + 1).t, _t(i + 2).t]));
          break;
        case "d8":
          csent[i].f.push(_f(["d8", _t(i - 2).t, _t(i - 1).t]));
          break;
        case "d3":
          csent[i].f.push(_f(["d3", _t(i + 2).t, _t(i + 3).t]));
          break;
        case "d7":
          csent[i].f.push(_f(["d7", _t(i - 3).t, _t(i - 2).t]));
          break;
        // character trigram
        case "t0":
          csent[i].f.push(_f(["t0", _t(i - 1).c, _t(i).c, _t(i + 1).c]));
          break;
        case "t1":
          csent[i].f.push(_f(["t1", _t(i).c, _t(i + 1).c, _t(i + 2).c]));
          break;
        case "t9":
          csent[i].f.push(_f(["t9", _t(i - 2).c, _t(i - 1).c, _t(i).c]));
          break;
        default:
          // if the feature template is a function,
          // invoke it and add the returned value
          if (typeof feat == "function") csent[i].f.push(_f(feat(_t, i)));
          else throw "Invalid feature specification!";
      }
    }
  }
};
...
RakutenMA.prototype.csent2feats = function (csent) {
  ...
  for (i = 0; i < csent.length; i++) {
    ...
    if (i != 0) feats.push(["t", csent[i].l, csent[i - 1].l]);
  }
  return feats;
};
...
RakutenMA.prototype.calc_states0 = function (cfeats, weights, e_def) {
  ...
  for (j in cfeats) {
    ...
    if (j == 0) for (k in cemits) states0[k] = true;

    if (cemits) {
      for (k in cemits) {
        if (k in scores0) scores0[k] += cemits[k].v;
        else scores0[k] = cemits[k].v;
      }
    }
  }
  // replace by scores
...
RakutenMA.prototype.decode = function (csent) {
  ...
  var statesp = {};
  statesp[RakutenMA._BEOS_LABEL] = {
    score: 0.0,
    path: [RakutenMA._BEOS_LABEL],
  };
  ...
  for (i = 1; i < csent.length; i++) {
    ...
    for (s0 in states0) {
      ...
      for (sp in statesp) {
        t_score = 0.0;
        if (sp in trans0) t_score = trans0[sp].v || 0.0;
      ...
      if (max_state && max_score > 0)
        states0[s0] = {
          score: max_score,
          path: statesp[max_state].path.concat([s0]),
        };
    }
    statesp = states0;
  }
...
RakutenMA.prototype.train_one = function (sent) {
  ...
  for (i in sent) sent_str += sent[i][0];
...
RakutenMA.create_ctype_chardic_func = function (chardic) {
  ...
  return function (str) {
    if (str in chardic) return chardic[str];
    else return [];
  };
};
...
RakutenMA.tokenize_corpus = function (tokenize_func, corpus) {
  ...
  for (i in corpus) {
    sent = corpus[i];
    sent_str = "";
    for (j in sent) sent_str += sent[j][0];
...
RakutenMA.eval_corpus = function (corpus_ans, corpus_sys) {
  ...
  return [
    (1.0 * tps) / tokens_sys, // precision
    (1.0 * tps) / tokens_ans, // recall
    (2.0 * tps) / (tokens_ans + tokens_sys),
  ]; // F1
};
...
Trie.find = function (trie, key, depth) {
  ...
  if (depth == key.length) {
    ...
    else return Trie.find(node, key, depth + 1);
  }
};
...
Trie.find_partial = function (trie, key, depth) {
  ...
  if (depth == key.length) {
    ...
    node = trie[key[depth]];
    if (!node) return undefined;
    else return Trie.find_partial(node, key, depth + 1);
  }
};
...
Trie.inner_prod = function (trie1, trie2) {
  ...
  if (trie1.v && trie2.v) res += trie1.v * trie2.v;
  for (key in trie1) {
    if (key == "v") continue;
    node = trie2[key];
    if (node) res += Trie.inner_prod(trie1[key], node);
  }
  return res;
};
...
Trie.mult = function (trie1, trie2) {
  ...
  var key;
  if (trie1.v && trie2.v) trie1.v *= trie2.v;

  for (key in trie2) {
    if (key == "v") continue;
    if (key in trie1) Trie.mult(trie1[key], trie2[key]);
  }
};
...
Trie.copy = function (trie) {
  ...
  var new_trie = {},
    key;
  for (key in trie) {
    if (key == "v") new_trie.v = trie.v;
    else new_trie[key] = Trie.copy(trie[key]);
  }
  return new_trie;
};
...
Trie.toString = function (trie, path) {
  ...
  if (!path) path = [];
  for (key in trie) {
    if (key == "v") res += path.join(" ") + "\t" + trie.v + "\n";
    else res += Trie.toString(trie[key], path.concat([key]));
  }
  return res;
};
...
Trie.each = function (trie, callback, path) {
  ...
  if (!path) path = [];
  for (key in trie) {
    if (key == "v") callback(path, trie.v);
    else Trie.each(trie[key], callback, path.concat([key]));
  }
};
...
SCW.prototype.calc_alpha = function (margin, variance) {
  ...
  var alpha =
    (-1 * margin * this.psi +
      this.phi * Math.sqrt(term1 * term1 + alpha_denom)) /
    alpha_denom;
...
if (typeof exports !== "undefined") module.exports = RakutenMA;
